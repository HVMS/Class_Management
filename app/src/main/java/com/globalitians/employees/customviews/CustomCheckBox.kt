...
class CustomCheckBox : AppCompatCheckBox {
    ...
    constructor(context: Context, attrs: AttributeSet?) : this(context, attrs, android.R.attr.checkboxStyle) {
        init(attrs)
    }
...
//        typeface = myTypeface
        CustomCheckBox(context, attrs, 0)
    }
    ...
    fun CustomCheckBox(
        ...
        try {
            val a: TypedArray =
                context.obtainStyledAttributes(attrs, R.styleable.CustomCheckBox, defStyle, 0)
            val customEnumValue: CustomEnumBrandonNew.CustomFontType =
                CustomEnumBrandonNew.CustomFontType.fromId(
                    a.getInt(
                        R.styleable.CustomCheckBox_font_type,
                        0
                    )
                )
            a.recycle()
